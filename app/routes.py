...
from schemas import responses as resp

from queries import *
import auth
router = APIRouter(prefix="/pet",tags=["animals"])

@router.get("/get_pets", response_model=List[resp.PetResponse], response_model_exclude_none=True)
...
@router.get("/get_pet/{id}", response_model=resp.PetResponse)
...
@router.post("/create_pet", response_model=resp.PetResponse)
