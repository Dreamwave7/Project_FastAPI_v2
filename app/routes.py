...
async def get_pets(db:Annotated[Session, Depends(get_db)]):
    result = await get_all_pets(db)
    return result

@router.get("/get_pet/{id}", response_model=PetResponse|None)
async def get_pet(id:int, db:Annotated[Session, Depends(get_db)]):
    result = await get_pet_from_id(db, pet_id=id)
    return result

@router.post("/create_pet", response_model=PetResponse)
async def create_pet(data: PetModel, db:Annotated[Session,Depends(get_db)]):
    new_pet = await create_pet_from_data(db,data)
    return new_pet
